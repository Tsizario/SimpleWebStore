...
namespace SimpleWebStore.UI.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            var rolesExist = _roleManager.RoleExistsAsync(Roles.Role_Admin).GetAwaiter().GetResult();
            ...
            if (!rolesExist)
            {
                ...
                for(int i = 0; i < roles.Length; i++)
                {
                    _roleManager.CreateAsync(
                        new IdentityRole(roles[i].GetValue(roles[i]).ToString()))
